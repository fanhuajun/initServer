...
./configure --prefix=$PREFIX --enable-server --enable-agent --with-mysql=/alidata/server/mysql/bin/mysql_config --enable-ipv6 --with-net-snmp
make
make install
...
export MYSQL_PASSWORD=fanhuajun   #fanhuajun
...
#fanhuajun
PHP_INI=/etc
sed -i 's/post_max_size = 8M/post_max_size = 16M/g' $PHP_INI/php.ini
sed -i 's/max_execution_time = 30/max_execution_time = 300/g' $PHP_INI/php.ini
sed -i 's/max_input_time = 60/max_input_time = 300/g' $PHP_INI/php.ini
sed -i 's/;date.timezone =/date.timezone = Asia\/Shanghai/g' $PHP_INI/php.ini
...
mkdir -p /var/www/default/zabbix
cp -r ./zabbix-2.4.2/frontends/php/* /var/www/default/zabbix/
chmod 777 /var/www/default/zabbix/conf/
service httpd restart
#modify zabbix passwd
sleep 1
# export ZPASS=$(date | md5sum |head -c 10)
export ZPASS=zabbix # fanhuajun
# /alidata/server/mysql/bin/mysql -uroot -p$MYSQL_PASSWORD -e "update zabbix.users set passwd=md5('$ZPASS') where userid='1';"
/alidata/server/mysql/bin/mysql -uroot -p$MYSQL_PASSWORD -e "update zabbix.users set passwd='$ZPASS' where userid='1';"
